...
OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./ashby-data)
  --format <fmt>        Output format: json, csv, table (default: json)
  --compensation        Include compensation data
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company posthog --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./ashby-data");
