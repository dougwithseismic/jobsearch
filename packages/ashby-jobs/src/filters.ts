import type { AshbyJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: AshbyJob, filter: JobFilter): boolean {
  ...
  if (filter.location) {
    const locationMatch =
      ...
      job.secondaryLocations?.some((l) => filter.location!.test(l.location)) ||
      (job.address?.postalAddress?.addressCountry &&
        filter.location.test(job.address.postalAddress.addressCountry)) ||
      (job.address?.postalAddress?.addressRegion &&
        filter.location.test(job.address.postalAddress.addressRegion)) ||
      (job.address?.postalAddress?.addressLocality &&
        filter.location.test(job.address.postalAddress.addressLocality));
