...
describe('toCSV', () => {
  it('produces valid CSV with correct headers', () => {
    ...
    expect(lines[0]).toBe(
      'company,title,department,team,location,remote,type,workplace,publishedAt,jobUrl,applyUrl,compensation'
    );
  ...
  it('handles commas in values by quoting', () => {
    ...
    const csv = toCSV(data);
    const lines = csv.split('\n');
    // Company and title should be quoted
  ...
  it('formats publishedAt as date only', () => {
    const data = [
      makeCompanyJobs({
        jobs: [makeJob({ id: '1', publishedAt: '2026-03-01T12:30:00.000Z' })],
        jobCount: 1,
      }),
    ];
    const csv = toCSV(data);
  ...
  it('handles empty compensation', () => {
    const data = [
      makeCompanyJobs({
        jobs: [makeJob({ id: '1', compensationTierSummary: undefined })],
        jobCount: 1,
      }),
    ];
    const csv = toCSV(data);
...
describe('toTable', () => {
  ...
  it('truncates long values with ellipsis', () => {
    ...
    const table = toTable(data);
    // Title maxW is 30 chars, so anything longer gets truncated
    expect(table).toContain('\u2026');
  });
