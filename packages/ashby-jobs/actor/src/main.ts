...
import { normalize } from '@jobsearch/job-ingest/src/normalizers/ashby.js';
  ...
  for (const company of results) {
    if (outputFormat === 'raw') {
      ...
      const batch = company.jobs.map((job) => ({
        ...
        secondaryLocations: job.secondaryLocations?.map((l) => l.location) ?? [],
        isRemote: job.isRemote,
        workplaceType: job.workplaceType,
        publishedAt: job.publishedAt,
        jobUrl: job.jobUrl,
        applyUrl: job.applyUrl,
        compensationTierSummary: job.compensationTierSummary ?? null,
