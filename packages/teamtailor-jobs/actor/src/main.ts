...
import { discoverSlugs, scrapeAll, scrapeCompany, searchJobs } from 'teamtailor-jobs';
import type { CompanyJobs, JobFilter } from 'teamtailor-jobs';
import { normalize } from '@jobsearch/job-ingest/src/normalizers/teamtailor.js';
  ...
  for (const company of results) {
    if (outputFormat === 'raw') {
      ...
      const batch = company.jobs.map((job) => ({
        ...
        link: job.link,
        guid: job.guid,
        ...(includeContent ? { description: job.description } : {}),
