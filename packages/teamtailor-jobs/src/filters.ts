import type { TeamtailorJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: TeamtailorJob, filter: JobFilter): boolean {
  if (filter.location) {
    const locationText = job.locations
      .map((l) => [l.name, l.city, l.country].filter(Boolean).join(" "))
      .join(" ");
  ...
  if (filter.role) {
    if (!filter.role.test(job.role ?? "")) return false;
  }
