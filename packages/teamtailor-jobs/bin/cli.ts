...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  Teamtailor job board scraper (RSS)\n`);
}
...
FILTER OPTIONS:
  ...
  --role <regex>        Filter by role pattern
  --keyword <regex>     Filter by title/content keyword

OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./teamtailor-data)
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company storytel --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./teamtailor-data");
...
async function runScrape(parsed: ParsedArgs): Promise<void> {
  ...
  // Apply limit (for non-single-company mode)
  if (!singleCompany) {
    const limitStr = flag(parsed.flags, "limit");
    if (limitStr) {
