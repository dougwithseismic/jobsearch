import type { JobviteJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: JobviteJob, filter: JobFilter): boolean {
  if (filter.location) {
  ...
  if (filter.category) {
    if (!filter.category.test(job.category)) return false;
  }
