...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  Jobvite job board scraper\n`);
}
...
COMMANDS:
  scrape                Scrape jobs from a Jobvite career page
  search <query>        Search previously scraped job data

SCRAPE OPTIONS:
  --company <slug>      Scrape a single company (required for scrape)
...
FILTER OPTIONS:
  ...
  --category <regex>    Filter by category pattern
  --keyword <regex>     Filter by title/content keyword

OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./jobvite-data)
  --format <fmt>        Output format: json, csv, table (default: json)
  --no-content          Exclude HTML job descriptions
...
EXAMPLES:
  ${NAME} scrape --company conair --format table
  ${NAME} scrape --company conair --limit 3
  ${NAME} scrape --slugs slugs.txt --location "remote"
  ${NAME} search "senior engineer" --department "engineering" --format table
`);
}
...
async function runScrape(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./jobvite-data");
  const format = flag(parsed.flags, "format") ?? "json";
  const concurrency = parseInt(flag(parsed.flags, "concurrency") ?? "5", 10);
  const includeContent = !hasFlag(parsed.flags, "no-content");
  ...
  if (singleCompany) {
    ...
    const result = await scrapeCompany(singleCompany, { includeContent, limit });
    ...
    console.error("Error: --company or --slugs is required for scrape command.");
...
function buildFilter(flags: Record<string, string | boolean>): JobFilter | null {
  ...
  const categoryStr = flag(flags, "category");
  const keywordStr = flag(flags, "keyword");

  if (!locationStr && !departmentStr && !categoryStr && !keywordStr) return null;

  const filter: JobFilter = {};
  if (locationStr) filter.location = new RegExp(locationStr, "i");
  if (departmentStr) filter.department = new RegExp(departmentStr, "i");
  if (categoryStr) filter.category = new RegExp(categoryStr, "i");
