import type { WorkableJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: WorkableJob, filter: JobFilter): boolean {
  ...
  if (filter.location) {
    const locationMatch =
      filter.location.test(job.country) ||
      filter.location.test(job.city) ||
      filter.location.test(job.state) ||
      job.locations?.some(
        (l) =>
          filter.location!.test(l.country) ||
          filter.location!.test(l.city) ||
          filter.location!.test(l.region) ||
          filter.location!.test(l.countryCode)
      );
  ...
  if (filter.keyword) {
    const text = `${job.title} ${job.descriptionHtml ?? ""}`;
