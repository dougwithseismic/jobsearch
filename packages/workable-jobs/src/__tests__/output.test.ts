...
describe('toCSV', () => {
  it('produces valid CSV with correct headers', () => {
    ...
    expect(lines[0]).toBe(
      'company,title,department,country,city,state,remote,type,experience,industry,publishedAt,jobUrl,applyUrl'
    );
  ...
  it('handles empty department', () => {
    const data = [
      makeCompanyJobs({
        jobs: [makeJob({ shortcode: '1', department: '' })],
        jobCount: 1,
      }),
    ];
    const csv = toCSV(data);
