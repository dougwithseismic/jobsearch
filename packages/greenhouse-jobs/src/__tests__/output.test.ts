...
describe('flattenJobs', () => {
  ...
  it('joins departments array into comma-separated string', () => {
    const data = [
      makeCompanyJobs({
        jobs: [
    ...
    const flat = flattenJobs(data);
    expect(flat[0].department).toBe('Engineering, Data Science');
  });

  it('joins offices array into comma-separated string', () => {
    const data = [
      makeCompanyJobs({
        jobs: [
          makeJob({ id: 1001, offices: ['Berlin', 'London'] }),
        ],
        jobCount: 1,
      }),
    ];
    const flat = flattenJobs(data);
    expect(flat[0].office).toBe('Berlin, London');
  });
});
...
describe('toCSV', () => {
  it('produces valid CSV with correct headers', () => {
    ...
    expect(lines[0]).toBe(
      'company,title,department,office,location,updatedAt,absoluteUrl,internalJobId'
    );
  ...
  it('formats updatedAt as date only', () => {
    const data = [
      makeCompanyJobs({
        jobs: [makeJob({ id: 1001, updatedAt: '2026-03-01T12:30:00.000Z' })],
        jobCount: 1,
      }),
    ];
    const csv = toCSV(data);
