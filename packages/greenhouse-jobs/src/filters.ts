import type { GreenhouseJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: GreenhouseJob, filter: JobFilter): boolean {
  if (filter.location) {
    if (!filter.location.test(job.location)) return false;
  }

  if (filter.department) {
    const deptText = job.departments.join(" ");
    if (!filter.department.test(deptText)) return false;
  }

  if (filter.office) {
    const officeText = job.offices.join(" ");
    if (!filter.office.test(officeText)) return false;
  }
