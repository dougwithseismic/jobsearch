...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  Greenhouse job board scraper\n`);
}
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company stripe --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./greenhouse-data");
...
function buildFilter(flags: Record<string, string | boolean>): JobFilter | null {
  ...
  if (!locationStr && !departmentStr && !officeStr && !keywordStr) return null;
