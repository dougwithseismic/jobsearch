...
import { discoverSlugs, scrapeAll, scrapeCompany, searchJobs } from 'greenhouse-jobs';
import type { CompanyJobs, JobFilter } from 'greenhouse-jobs';
import { normalize } from '@jobsearch/job-ingest/src/normalizers/greenhouse.js';
  ...
  for (const company of results) {
    if (outputFormat === 'raw') {
      ...
      const batch = company.jobs.map((job) => ({
        ...
        metadata: job.metadata.map((m) => ({ name: m.name, value: m.value })),
        ...(includeContent ? { content: job.content ?? null } : {}),
