import type { BreezyJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: BreezyJob, filter: JobFilter): boolean {
  if (filter.location) {
  ...
  if (filter.keyword) {
    const text = `${job.name} ${job.department}`;
    if (!filter.keyword.test(text)) return false;
  }

  if (filter.salary) {
    if (!filter.salary.test(job.salary)) return false;
  }
