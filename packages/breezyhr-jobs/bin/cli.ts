...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  BreezyHR job board scraper\n`);
}
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company hubstaff --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./breezyhr-data");
...
function buildFilter(flags: Record<string, string | boolean>): JobFilter | null {
  ...
  const salaryStr = flag(flags, "salary");
  const remoteOnly = hasFlag(flags, "remote");

  if (!locationStr && !departmentStr && !keywordStr && !salaryStr && !remoteOnly) return null;
  ...
  if (salaryStr) filter.salary = new RegExp(salaryStr, "i");
  if (remoteOnly) filter.remoteOnly = true;
