import type { SmartRecruitersJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: SmartRecruitersJob, filter: JobFilter): boolean {
  if (filter.remote && !job.location?.remote) {
    return false;
  }

  if (filter.location) {
    const locationMatch =
      (job.location?.city && filter.location.test(job.location.city)) ||
      (job.location?.region && filter.location.test(job.location.region)) ||
      (job.location?.country && filter.location.test(job.location.country));
    if (!locationMatch) return false;
  }

  if (filter.department) {
    if (!filter.department.test(job.department?.label ?? "")) return false;
  }

  if (filter.keyword) {
    const text = `${job.name} ${job.descriptionHtml ?? ""}`;
