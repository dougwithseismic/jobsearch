...
describe('filterCompanyJobs', () => {
  ...
  it('matches location against country', () => {
    const company = makeCompanyJobs();
    const result = filterCompanyJobs(company, { location: /Germany/i });
    expect(result).not.toBeNull();
    expect(result!.jobs).toHaveLength(1);
    expect(result!.jobs[0].id).toBe('1');
  });

  it('matches location against region', () => {
    const company = makeCompanyJobs();
  ...
  it('filters by department regex', () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.jobs).toHaveLength(1);
    expect(result!.jobs[0].department.label).toBe('Product');
  });
  ...
  it('filters by keyword matching descriptionHtml', () => {
    const company = makeCompanyJobs({
      jobs: [
        ...
        makeJob({ id: '2', name: 'Another Role' }),
      ],
      jobCount: 2,
    });
  ...
  it('applies combined filters (remote + location)', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({ id: '1', location: { city: 'Berlin', region: 'Berlin', country: 'Germany', remote: true } }),
        makeJob({ id: '2', location: { city: 'New York', region: 'NY', country: 'US', remote: true } }),
        makeJob({ id: '3', location: { city: 'SF', region: 'CA', country: 'US', remote: false } }),
      ],
      jobCount: 3,
    });
  ...
  it('handles regex special characters in location filter', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({ id: '1', location: { city: 'New York (NYC)', region: 'NY', country: 'US', remote: false } }),
      ],
      jobCount: 1,
    });
    const result = filterCompanyJobs(company, {
...
describe('searchResults', () => {
  const data = [
    makeCompanyJobs({
      ...
      slug: 'acme',
      jobs: [
        makeJob({ id: '1', name: 'Frontend Engineer', department: { label: 'Engineering' } }),
        makeJob({ id: '2', name: 'Backend Engineer', department: { label: 'Engineering' } }),
      ],
      jobCount: 2,
    }),
    makeCompanyJobs({
      ...
      slug: 'widget',
      jobs: [
        makeJob({ id: '3', name: 'Product Designer', department: { label: 'Design' } }),
      ],
      jobCount: 1,
    }),
  ];
  ...
  it('searches by text matching job title (Designer)', () => {
  ...
  it('combines text and structured filters', () => {
    ...
    expect(result).toHaveLength(1);
    expect(result[0].jobs.every((j) => j.location.remote)).toBe(true);
  });
