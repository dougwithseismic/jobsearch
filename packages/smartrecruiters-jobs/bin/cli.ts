...
const VERSION = "1.0.0";
const NAME = "smartrecruiters-jobs";
...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  SmartRecruiters job board scraper\n`);
}
...
OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./smartrecruiters-data)
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company Spotify --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./smartrecruiters-data");
