...
describe('filterCompanyJobs', () => {
  ...
  it('detects remote from workplaceType field', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({ id: '1', workplaceType: 'remote', location: 'Anywhere', allLocations: ['Anywhere'] }),
        makeJob({ id: '2', workplaceType: 'on-site', location: 'NYC', allLocations: ['NYC'] }),
      ],
      jobCount: 2,
    });
    const result = filterCompanyJobs(company, { remote: true });
  ...
  it('detects remote from location text', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({ id: '1', workplaceType: 'unspecified', location: 'Remote - US' }),
      ],
      jobCount: 1,
    });
    const result = filterCompanyJobs(company, { remote: true });
  ...
  it('detects remote from allLocations', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({
          id: '1',
  ...
  it('matches location against allLocations', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({
          id: '1',
          location: 'New York',
          allLocations: ['New York', 'Berlin, Germany'],
        }),
      ],
      jobCount: 1,
    });
  ...
  it('matches department against team field too', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({ id: '1', department: 'Engineering', team: 'Platform' }),
        makeJob({ id: '2', department: 'Design', team: 'Product Design' }),
      ],
      jobCount: 2,
    });
    const result = filterCompanyJobs(company, { department: /Platform/i });
    expect(result).not.toBeNull();
    expect(result!.jobs).toHaveLength(1);
    expect(result!.jobs[0].id).toBe('1');
  });
  ...
  it('applies combined filters (remote + location)', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({ id: '1', workplaceType: 'remote', location: 'Remote - Europe', allLocations: ['Remote - Europe'] }),
        makeJob({ id: '2', workplaceType: 'remote', location: 'Remote - US', allLocations: ['Remote - US'] }),
        makeJob({ id: '3', workplaceType: 'on-site', location: 'San Francisco, CA', allLocations: ['San Francisco, CA'] }),
      ],
      jobCount: 3,
    });
  ...
  it('handles regex special characters in location filter', () => {
    const company = makeCompanyJobs({
      jobs: [
        makeJob({ id: '1', location: 'New York (NYC)' }),
      ],
      jobCount: 1,
    });
    const result = filterCompanyJobs(company, {
...
describe('searchResults', () => {
  ...
  it('searches by text matching designer title', () => {
  ...
  it('combines text and structured filters', () => {
    ...
    expect(result).toHaveLength(1);
    // All fixtures have workplaceType=remote by default
