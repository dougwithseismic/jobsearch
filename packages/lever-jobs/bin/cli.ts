...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  Lever job board scraper\n`);
}
...
OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./lever-data)
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./lever-data");
