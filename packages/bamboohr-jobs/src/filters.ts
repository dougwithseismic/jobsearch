import type { BambooHRJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: BambooHRJob, filter: JobFilter): boolean {
  if (filter.location) {
    const locationText = [job.location.city, job.location.state, job.location.addressCountry].filter(Boolean).join(" ");
    if (!filter.location.test(locationText)) return false;
  }

  if (filter.department) {
    if (!filter.department.test(job.departmentLabel)) return false;
  }

  if (filter.keyword) {
    const text = `${job.jobOpeningName} ${job.description ?? ""}`;
