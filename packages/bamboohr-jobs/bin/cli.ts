...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  BambooHR job board scraper\n`);
}
...
SCRAPE OPTIONS:
  ...
  --limit <n>           Limit number of companies to scrape
...
OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./bamboohr-data)
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company rippling --format table
  ${NAME} scrape --slugs slugs.txt --location "utah"
  ${NAME} search "engineer" --department "engineering" --format table
`);
}
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./bamboohr-data");
...
async function runScrape(parsed: ParsedArgs): Promise<void> {
  ...
  if (singleCompany) {
    ...
    const result = await scrapeCompany(singleCompany, { includeContent });
...
function buildFilter(flags: Record<string, string | boolean>): JobFilter | null {
  ...
  if (!locationStr && !departmentStr && !keywordStr) return null;
