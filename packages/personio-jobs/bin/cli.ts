...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  Personio job board scraper\n`);
}
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company n26 --format table
  ${NAME} scrape --slugs slugs.txt --location "berlin|munich"
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./personio-data");
...
async function runScrape(parsed: ParsedArgs): Promise<void> {
  ...
  const includeContent = hasFlag(parsed.flags, "content");
  const language = flag(parsed.flags, "language") ?? "en";
  ...
  if (singleCompany) {
    ...
    const result = await scrapeCompany(singleCompany, { includeContent, language });
...
function buildFilter(flags: Record<string, string | boolean>): JobFilter | null {
  ...
  const officeStr = flag(flags, "office");
  const keywordStr = flag(flags, "keyword");
  const seniorityStr = flag(flags, "seniority");
  const employmentStr = flag(flags, "employment");

  if (!locationStr && !departmentStr && !officeStr && !keywordStr && !seniorityStr && !employmentStr) return null;

  const filter: JobFilter = {};
  if (locationStr) filter.location = new RegExp(locationStr, "i");
  if (departmentStr) filter.department = new RegExp(departmentStr, "i");
  if (officeStr) filter.office = new RegExp(officeStr, "i");
  if (keywordStr) filter.keyword = new RegExp(keywordStr, "i");
  if (seniorityStr) filter.seniority = new RegExp(seniorityStr, "i");
  if (employmentStr) filter.employmentType = new RegExp(employmentStr, "i");
