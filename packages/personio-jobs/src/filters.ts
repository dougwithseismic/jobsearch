import type { PersonioJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: PersonioJob, filter: JobFilter): boolean {
  if (filter.location) {
    if (!filter.location.test(job.office)) return false;
  }

  if (filter.department) {
    if (!filter.department.test(job.department)) return false;
  }

  if (filter.office) {
    if (!filter.office.test(job.office)) return false;
  }

  if (filter.keyword) {
    const descText = job.jobDescriptions.map((d) => d.value).join(" ");
    const text = `${job.name} ${job.keywords} ${descText}`;
    if (!filter.keyword.test(text)) return false;
  }

  if (filter.seniority) {
    if (!filter.seniority.test(job.seniority)) return false;
  }

  if (filter.employmentType) {
    if (!filter.employmentType.test(job.employmentType)) return false;
  }
