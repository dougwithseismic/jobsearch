...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  JazzHR job board scraper\n`);
}
...
SCRAPE OPTIONS:
  ...
  --descriptions        Include full HTML job descriptions (fetches detail pages)
...
OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./jazzhr-data)
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company nro --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./jazzhr-data");
