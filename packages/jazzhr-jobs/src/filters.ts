import type { JazzHRJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: JazzHRJob, filter: JobFilter): boolean {
  if (filter.location) {
  ...
  if (filter.keyword) {
    const text = `${job.title} ${job.description ?? ""} ${job.descriptionHtml ?? ""}`;
