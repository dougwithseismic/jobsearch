...
import { discoverSlugs, scrapeAll, scrapeCompany, searchJobs } from 'pinpoint-jobs';
import type { CompanyJobs, JobFilter } from 'pinpoint-jobs';
import { normalize } from '@jobsearch/job-ingest/src/normalizers/pinpoint.js';
  ...
  for (const company of results) {
    if (outputFormat === 'raw') {
      ...
      const batch = company.jobs.map((job) => ({
        ...
        url: job.url,
        compensationVisible: job.compensationVisible,
