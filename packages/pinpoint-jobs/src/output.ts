  ...
  for (const company of results) {
    for (const job of company.jobs) {
      rows.push({
        ...
        workplaceType: job.workplaceTypeText,
        employmentType: job.employmentTypeText,
        url: job.url,
        content: job.content,
      });
    }
    ...
    for (const job of company.jobs) {
      rows.push(
        [
          ...
          csvQuote(job.workplaceTypeText),
          csvQuote(job.employmentTypeText),
          job.url,
        ].join(",")
      );
    }
...
export function toTable(results: CompanyJobs[], _maxWidth = 120): string {
  ...
  const columns: { key: keyof FlatJob; label: string; maxW: number }[] = [
    ...
    { key: "workplaceType", label: "Workplace", maxW: 14 },
    { key: "employmentType", label: "Type", maxW: 12 },
  ];
