import type { PinpointJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: PinpointJob, filter: JobFilter): boolean {
  if (filter.location) {
    const locText = `${job.location.name} ${job.location.city} ${job.location.province}`;
    if (!filter.location.test(locText)) return false;
  }
  ...
  if (filter.keyword) {
    const text = `${job.title} ${job.content ?? ""} ${job.description ?? ""}`;
