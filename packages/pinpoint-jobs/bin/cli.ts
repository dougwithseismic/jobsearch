...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  Pinpoint job board scraper\n`);
}
...
OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./pinpoint-data)
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company workwithus --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./pinpoint-data");
