...
function banner() {
  console.log(`\n  ${NAME} v${VERSION}`);
  console.log(`  Recruitee job board scraper\n`);
}
...
FILTER OPTIONS:
  ...
  --country <regex>     Filter by country pattern

OUTPUT OPTIONS:
  --output, -o <dir>    Output directory (default: ./recruitee-data)
...
EXAMPLES:
  ${NAME} discover
  ${NAME} scrape --company bynder --format table
...
async function runDiscover(parsed: ParsedArgs): Promise<void> {
  ...
  const outputDir = resolve(flag(parsed.flags, "output") ?? "./recruitee-data");
...
function buildFilter(flags: Record<string, string | boolean>): JobFilter | null {
  ...
  const countryStr = flag(flags, "country");

  if (!remote && !locationStr && !departmentStr && !keywordStr && !countryStr) return null;
  ...
  if (countryStr) filter.country = new RegExp(countryStr, "i");
