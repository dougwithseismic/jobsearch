import type { RecruiteeJob, CompanyJobs, JobFilter } from "./types.js";

/**
 * Test if a single job matches a filter.
 */
function jobMatchesFilter(job: RecruiteeJob, filter: JobFilter): boolean {
  if (filter.remote && !job.remote) {
    return false;
  }
  ...
  if (filter.location) {
    const locationMatch =
      ...
      (job.city && filter.location.test(job.city)) ||
      (job.state && filter.location.test(job.state)) ||
      (job.country && filter.location.test(job.country));
    if (!locationMatch) return false;
  }

  if (filter.country) {
    if (!filter.country.test(job.country ?? "")) return false;
  }
